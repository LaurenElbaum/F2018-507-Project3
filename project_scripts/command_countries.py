...
#
def prepare_countries_output_row(data, percent, count):
    output = (
...
#
def process_countries_command(params):
    ...
    sql1 = 'select C1.EnglishName As country, C1.Region AS region,'
    sql2 = ' FROM Bars Join Countries AS C1 ON Bars'
    sql3 = '=C1.Id'
    sql4 = ' GROUP BY country HAVING COUNT(*) > 4'

    # default values as specified by spec!!
    where      = ''                   # No default - dont require region filter
    ...
    for option in params:
        ...
        if whereSet > 1 or limitSet > 1 or orderBySet > 1 or selectSet > 1:
            return('error', 'Multiple filter options not allowed.')


    # concactenate sql stubs and param interpretations for query
    sql = sql1 + column + sql2 + select + sql3 + where + sql4 + orderBy + limit
